# Backend-despachos-Pipeline-CI-CD-evaluacion-2

Backend de gestión de despachos desarrollado con **Spring Boot 3.4.4** y desplegado mediante un pipeline CI/CD en una instancia EC2 privada de AWS.
...
## 🌐 Backend Despachos(app) desplegado en AWS
...
> **Nota:** el `Dockerfile` se encuentra dentro de la carpeta `Springboot-API-REST-DESPACHO/` junto al código fuente.
...
2. **Stage 2 - Runner:** usa solo el JRE (`eclipse-temurin:17-jre-alpine`) en lugar del JDK completo, reduciendo el tamaño de la imagen final. Se crea un usuario no-root por seguridad.
...
Se definió un servicio `backend-despachos` con:
- Imagen publicada en Docker Hub: `gonzalo25u/backend-despachos:latest`
- Puerto `8080:8081` (el host expone el 8080, Tomcat corre internamente en el 8081)
- Variables de entorno para conexión a MySQL en EC2 `data`
- **Named volume** para logs persistentes: `despachos_logs:/app/logs`
...
4. Build desde la subcarpeta `./Springboot-API-REST-DESPACHO` y push con dos tags:
...
**Problema:** el código fuente está dentro de la subcarpeta `Springboot-API-REST-DESPACHO/` y no en la raíz del repositorio, por lo que el pipeline no encontraba el `Dockerfile` ni el `pom.xml`.

**Solución:** se configuró el `context` del build en el `ci-cd.yml` para apuntar a la subcarpeta `./Springboot-API-REST-DESPACHO`. El `Dockerfile` también se movió dentro de esa subcarpeta para que el build lo encuentre correctamente.

---

### 2. Puerto incorrecto en docker-compose
**Problema:** el `docker-compose.yml` mapeaba el puerto `8080:8080`, pero Tomcat dentro del contenedor corre en el puerto `8081`. Esto causaba que el contenedor no respondiera correctamente.

**Solución:** se corrigió el mapeo a `8080:8081` — el host expone el 8080 y lo redirige al 8081 interno del contenedor.

---

### 3. Error de autenticación MySQL (Public Key Retrieval)
**Problema:** al conectarse a MySQL 8.0, Spring Boot lanzaba el error `Public Key Retrieval is not allowed`. Esto ocurre porque MySQL 8.0 usa por defecto el plugin `caching_sha2_password`.
...
**Problema:** el paso de `scp` en el pipeline fallaba porque el directorio `/home/ec2-user/backend-despachos/` no existía en la EC2 `app`.
...
**Problema:** el pipeline fallaba con `Host key verification failed` al intentar conectarse via jump host.
...
git clone https://github.com/gonzalo25u/Backend-despachos-Pipeline-CI-CD-evaluacion-2-.git
